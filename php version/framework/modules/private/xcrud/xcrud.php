<?php
/** f0ska xCRUD v.1.6.17; 04/2014 */
error_reporting(E_ALL); // great for debug and development, you can remove this two lines for production app
...
class Xcrud
{
    ...
    public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
    {
        if ($instance_name && $field && $inner_tbl && $tbl_field)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                $instance->inner_where[$fitem['table'] . '.' . $fitem['field']] = $inner_tbl . '.' . $tbl_field; // this connects nested table with parent
                ...
                switch ($type)
                {
                    ...
                    case 'price':
                        $def_attr = array(
                            ...
                            'suffix' => '',
                            'point' => '.');
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, $attr);
                        break;
                    case 'select':
                    case 'multiselect':
                    case 'radio':
                    ...
                    switch ($this->field_type[$key])
                    {
                        ...
                        case 'datetime':
                            if ($val !== '')
                            {
                                ...
                                {
                                    $val = gmdate('Y-m-d H:i:s', (int)$val);
                                }
                            }
                            else
    ...
    {
        $del = false;
        if (!$this->is_remove || $this->table_ro)
            return self::error('Forbidden');
        if ($this->before_remove)
        {
            ...
            {
                call_user_func_array($this->before_remove['callable'], array($this->primary_val,$this->table));
            }
        }
        ...
        if ($this->after_remove)
        {
            ...
            {
                call_user_func_array($this->after_remove['callable'], array($this->primary_val));
            }
        }
        return $del;
    }
    ...
    protected function validate_postdata($postdata)
    {
        foreach ($postdata as $key => $val)
        {
            ...
            elseif (isset($this->validation_pattern[$key]))
            {
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        if (!$this->table_name && !$this->query)
            $this->table_name = $this->_humanize(mb_substr($this->table, strlen($this->prefix)));
        if ($task)
        {
            switch ($task)
            {
    ...
    protected function create_bool($name, $value = '', $tag = array())
    {
        ...
        return $this->single_tag($tag, $this->theme_config('bool_field'), $this->field_attr[$name], true);
    }
    ...
    protected function create_radio($name, $value = '', $tag = array())
    {
        ...
        if (is_array($this->field_attr[$name]['values']))
        {
            foreach ($this->field_attr[$name]['values'] as $optkey => $opt)
            {
                ...
                $out .= $this->html_safe($opt) . $this->close_tag($label_tag);
            }
        }
        else
        {
    ...
    protected function create_fk_relation($name, $value = '', $tag = array())
    {
        ...
        {
            $name_select = 'CONCAT_WS(' . $db->escape($this->fk_relation[$name]['rel_separator']) . ',`' . implode('`,`', $this->
                fk_relation[$name]['rel_tbl'] . '`.`' . $this->fk_relation[$name]['rel_name']) . '`) AS `name`';
        }
        else
        {
    ...
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        if ($value)
        {
            ...
            $attr = array('src' => $this->file_link($name, $this->primary_val, (isset($this->upload_config[$name]['detail_thumb']) ?
                    $this->upload_config[$name]['detail_thumb'] : false)), 'alt' => '');
        ...
        if (isset($this->field_type[$field]))
        {
            switch ($this->field_type[$field])
            {
                ...
                case 'image':
                    if ($value)
                    {
                        if (Xcrud_config::$images_in_grid)
                        {
                            ...
                            $out .= $this->single_tag('img', '', array(
                                'alt' => '',
                                'src' => $this->file_link($field, $primary_val, $thumb),
                    ...
                    $image = $this->file_link($field, $primary_val);
                    break;
                case 'remote_image':
                    if ($value)
                    {
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        if (!isset($this->hide_button['duplicate']) && !$this->table_ro && $this->is_duplicate($row))
        {
            $tag = array(
                'tag' => 'a',
                'class' => 'xcrud-action ' . $this->theme_config('grid_default'),
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false)
    {
    ...
    protected function additional_fields($fields = '')
    {
        if ($fields)
        {
            ...
            foreach ($fdata as $key => $fitem)
            {
                if (!isset($this->subselect[$key]) && $fitem['field'] != 'value')
                {
                    if (!isset($this->fields[$key]))
                    {
    ...
    protected function replace_text_variables($value, $data, $safety = false, $null_if_empty = false)
    {
            ...
            '&uarr; ', 'desc' => '&darr; '))
    {
        ...
        foreach ($this->columns as $field => $fitem)
        {
            //if (!$this->check_row_visibility($field))
            //    continue;
    ...
    {
        if ($this->is_add && !isset($this->hide_button['add']) && !$this->table_ro)
        {
...
class Xcrud_postdata_item
{
    ...
    function as_date()
    {
        return preg_match('/[^0-9\-]/u', $this->item) ? $this->item : gmdate('Y-m-d', (int)$this->item);
    }
    function as_datetime()
    {
        return preg_match('/[^0-9\-]/u', $this->item) ? $this->item : gmdate('Y-m-d H:i:s', (int)$this->item);
    }
    function as_time()
    {
        return preg_match('/[^0-9\-]/u', $this->item) ? $this->item : gmdate('H:i:s', (int)$this->item);
    }
